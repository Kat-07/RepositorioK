class Producto:
    ...
    def restar_cantidad(self, cantidad: int):
        ...
        if disponible:
            if cantidad > self.getCantidad_disponible():
                return self.getCantidad_disponible()
...
class Dispensadora:
    ...
    def realizar_venta(self, item: object, cantidad: int):
        cantidad_restante = item.restar_cantidad(cantidad)
        if cantidad_restante == True:
            self.__total_ventas += item.getPrecio() * cantidad
            return print('Venta realizada')
        else:
           cantidad_disponible = cantidad_restante
           return print(f'Venta no realizada. Cantidad disponible: {cantidad_disponible}')
